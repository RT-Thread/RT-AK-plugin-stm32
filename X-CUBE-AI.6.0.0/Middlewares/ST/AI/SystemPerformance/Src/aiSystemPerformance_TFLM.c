...
static uint32_t _current_time_ticks_cb(void)
{
  return dwtGetCycles();
}
...
{
  // struct tflm_c_tensor_info* t_info = &node->output[0];

  if (cookie) {
    struct u_node_stat* stat = (struct u_node_stat*)cookie;
    stat[node->node_info.idx].name = node->node_info.name;
    stat[node->node_info.idx].dur += node->node_info.dur;
  }

#if 0
  if (node->node_info.n_outputs)
    printf("CB %s:%d %d %d, %d:(%d, %d, %d) %d\r\n", node->node_info.name, (int)node->node_info.idx, (int)node->node_info.dur,
        (int)node->node_info.n_outputs,
        (int)t_info->idx, (int)t_info->height, (int)t_info->width, (int)t_info->channels, (int)t_info->type);
  else
    printf("CB %s:%d %d\r\n", node->node_info.name, (int)node->node_info.idx, (int)node->node_info.dur);
#endif
...
static TfLiteStatus observer_init(struct tflm_context *ctx)
{
  ...
  if (res != kTfLiteOk) {
    printf("E: Unable to register the callback observer..\r\n");
    return kTfLiteError;
  }
  return kTfLiteOk;
}
...
static void observer_done(struct tflm_context *ctx)
{
  ...
  if (_observer_options.cookie && p_info.node_dur) {
    ...
    const char *fmt = " %-6d%-25s %4d.%03d %6.02f %c\r\n";
...
static int aiTestPerformance(void)
{
  ...
  for (iter = 0; iter < niter; iter++) {
    ...
    if (res != kTfLiteOk) {
      printf("E: tflm_c_invoke() fails err=%d\r\n", (int)res);
      return res;
    }
  ...
  tmin = tmin - p_info.cb_dur / (uint64_t)iter;
  tmax = tmax - p_info.cb_dur / (uint64_t)iter;
#endif
  ...
  printf("Results (TFLM), %d inferences @%dMHz/%dMHz\r\n",
      (int)iter,
...
#if defined(USE_OBSERVER) && USE_OBSERVER == 1
  // dwtCyclesToTime(p_info.node_dur / (uint64_t)iter, &t);
  // printf(" duration     : %d.%03d ms (average)\r\n", t.s * 1000 + t.ms, t.us);
#endif

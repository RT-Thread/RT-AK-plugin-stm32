...
#define AI_RELOC_RT_VERSION_MAJOR 1
...
#define _INFO(name)     MAKE_FN_(name, _get_info)
...
#define _CREATE_        _CREATE(C_NAME)

static ai_bool ai_network_init_v2(ai_handle hdl, const ai_handle weights, const ai_handle act)
{
  const ai_network_params params = AI_NETWORK_PARAMS_INIT(
      _DATA_WEIGHTS(C_NAME_UP)(weights),
      _DATA_ACTIVATIONS(C_NAME_UP)(act) );
...
struct ai_reloc_network_entries {
    ...
    ai_bool (*info)(ai_handle network, ai_network_report* report);
 ...
 * @param   weights   address of the weights buffer
 * @param   act       address of the activations buffer
 *
